...
class LeRobotSO101DataConfig(DataConfigFactory):
    """Data config for the SO-101 6-DoF arm with three cameras (front/top/wrist).

    Designed for LeRobot v3.0 datasets where the per-step task field is used as the prompt.
    """
    ...
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Remap LeRobot's nested keys to the flat schema expected by SO101Inputs.
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
        ...
        # SO-101 raw actions are absolute joint positions; train on deltas for arm DoFs,
        # keep gripper (last channel) absolute.
...
_CONFIGS = [
    ...
    #
    # SO-101 (Ali's robot-arm-filtered dataset) — pi0.5 LoRA fine-tune.
    # Tuned for a single RTX A6000 (48GB). See runpod/README.md for the launcher.
    #
    TrainConfig(
